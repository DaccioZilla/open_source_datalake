...
def get_history(ticker, ds = None, ds_nodash = None):
    ...
    minio_client.put_object(minio_config["dest_bucket"], f'bronze/crypto/{ticker}/{ticker}_{ds_nodash}.csv', data=data_stream, length=len(data_bytes), content_type='application/csv')
    sleep(10)

@dag(
    schedule_interval= '0 0 * * *', #Cron expression Meia noite [0 0], todos os dias do mês [* *], totod os dias da semana [*]
